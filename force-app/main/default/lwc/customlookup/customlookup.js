   ...
   // initial function to populate default selected lookup record if defaultRecordId provided  
    connectedCallback(){
         if(this.defaultRecordId != ''){
            fetchDefaultRecord({ recordId: this.defaultRecordId , 'sObjectApiName' : this.sObjectApiName, whereclause : '$whereclause' })
            .then((result) => {
                if(result != null){
    ...
    @wire(fetchLookupData, { searchKey: '$searchKey' , sObjectApiName : '$sObjectApiName', whereclause : '$whereclause' })
     searchResult(value) {
