...
export default class Quiz1SummaryV2 extends NavigationMixin(LightningElement) {
...
fem = null;
    ...
    getStudentRecords(){
        ...
        getStudentRecords({
            ...
            if(responseWrapper.submittedStudentList !== undefined)
            {
                ...
                console.log('tableData',responseWrapper.submittedStudentList);
                // let index = 0;
                ...
                // }
                    ...
                    this.tableData.forEach(item => {
                        ...
                        if(item.Self_Awareness_and_Feedbacks__r && item.Self_Awareness_and_Feedbacks__r.length > 0){
                             ...
                             }else if(this.grd == 'Grade 11'){
                                 item.submission = item.Self_Awareness_and_Feedbacks__r[0].Grade11_Quiz1_Submitted__c;
                             }else if(this.grd == 'Grade 12'){
                                 item.submission = item.Self_Awareness_and_Feedbacks__r[0].Grade12_Quiz1_Submitted__c;
                             }
    ...
    addAssementHandler(event){
        ...
        .then(result => {
            ...
            let rxError;
            if(this.isEnglish){
                this.errorTitle = 'Quiz 1';
                this.rxError = 'Error while receiving student records';
            }else{
                this.errorTitle = 'प्रश्नावली 1';
                this.rxError = 'छात्र रिकॉर्ड प्राप्त करते समय त्रुटि';
            }
            if (Array.isArray(error.body)) {
                rxError = error.body.map(e => e.message).join(', ');
    ...
    nextNavigateToInternalPage() {
        ...
        this[NavigationMixin.Navigate]({
            type: 'comm__namedPage',
            attributes: {
                name: 'Quiz_1_Student_details_V2__c'
            },
            state: {
                fem : encodeURI(this.fem),
                sch : encodeURI(this.sch),
