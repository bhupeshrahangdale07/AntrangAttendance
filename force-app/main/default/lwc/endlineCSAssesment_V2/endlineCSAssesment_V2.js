...
export default class EndlineCSAssesment_V2 extends NavigationMixin(LightningElement) {
    ...
    addAssementHandler(event){
        ...
        var studentId = encodeURI(event.target.value);
            ...
            .then(result => {
                ...
                let rxError;
                if(this.isEnglish){
                    this.errorTitle = 'Endline Career Skills Assessment';
                    this.rxError = 'Error while receiving student records';
                }else{
                    this.errorTitle = 'एंडलाइन कैरियर कौशल मूल्यांकन';
                    this.rxError = 'छात्र रिकॉर्ड प्राप्त करते समय त्रुटि';
                }
                if (Array.isArray(error.body)) {
                    rxError = error.body.map(e => e.message).join(', ');
