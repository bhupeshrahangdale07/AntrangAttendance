({
	doInit : function(component, event, helper) {
        ...
        //CUSTOM CODE : For Dynamic Validation when value are been loaded from contact or child cmp loaded
        if( component.get('v.fieldType') == 'Label'){
                debugger;
                //When Values are preffiled from contact and we need do Validation on the values when they are loaded.
                if(component.get('v.contactFieldApiValue')){
                    let patternToMatch = new RegExp(component.get('v.validationPattern'));                   
                    let questionObject = component.get('v.questionObject');
                    questionObject.isValidComponent =  patternToMatch.test(component.get('v.contactFieldApiValue'));;
